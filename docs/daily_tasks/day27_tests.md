...
tests/stress_test.py              ← if it already exists, read it
...
1. What are the four section names in `competition_memory.json`? Quote them exactly.
2. What is `EscalationLevel.HITL` in `circuit_breaker.py`? At what `failure_count` does it fire?
3. What is `EscalationLevel.TRIAGE`? At what budget fraction does it fire?
4. What does `_build_critic_calibration()` return when the critic missed a large gap? Quote the `calibration_verdict` value.
5. Does `run_post_mortem_agent()` raise on missing `metrics.json`, or return an error dict?

Do not write any tests until all five are answered from the actual code.

---

## FILE 1 — `tests/test_day27_quality.py`
...
```bash
pytest tests/test_day27_quality.py -v --tb=short

# Stress tests (slow — run manually)
